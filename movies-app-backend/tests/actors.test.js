...
test('POST /actors - Crear un nuevo actor', async () => {
    ...
    const res = await request(app).post('/api/actors').send(newActor)
    id = res.body.id
...
test('PUT /actors/:id - Actualizar un actor', async () => {
    ...
    const res = await request(app).put(`/api/actors/${id}`).send(updatedActor)
    expect(res.status).toBe(200)
...
test('DELETE /actors/:id - Eliminar un actor por ID', async () => {
    const res = await request(app).delete(`/api/actors/${id}`)
