...
test('POST /directors - Crear un nuevo director', async () => {
    ...
    const res = await request(app).post('/api/directors').send(newDirector)
    id = res.body.id
...
test('PUT /directors/:id - Actualizar un director', async () => {
    ...
    const res = await request(app).put(`/api/directors/${id}`).send(updatedDirector)
    expect(res.status).toBe(200)
...
test('DELETE /directors/:id - Eliminar un director por ID', async () => {
    const res = await request(app).delete(`/api/directors/${id}`)
